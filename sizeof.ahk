...
sizeof(_TYPE_,parent_offset:=0,ByRef _align_total_ := 0){
      ...
      LoopParse,%A_LoopField%,`,`;,`t%A_Space%%A_Tab%
      {
        ...
        If (A_LoopField){ ; skip empty lines
            ...
            If Instr(A_LoopField,"{"){ ; Union, also check if it is a structure
              _union_++,_struct_.InsertAt(_union_,RegExMatch(A_LoopField,"i)^\s*struct\s*\{"))
  ...
  LoopParse,%_TYPE_%,`,`; ;,%A_Space%%A_Tab%`n`r
  {
    ...
    If InStr(_LF_,"*"){ ; It's a pointer, size will be always A_PtrSize
      ...
      RegExMatch(_LF_,"^(?<ArrType>[\w\d\._#@]+)?\s*(?<ArrName>[\w\d\._#@]+)?\s*\[?(?<ArrSize>\d+)?\]?\s*$",_)
      ...
      else _padding_:=sizeof(_defobj_?_defobj_:sizeof_get_global(_ArrType_)),_align_total_:=_max_size_:=sizeof_maxsize(_defobj_?_defobj_:sizeof_get_global(_ArrType_))
      if (_max_size_){
